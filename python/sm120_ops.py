...
try:
    ...
    warnings.warn(
        f"SM120 operations library not found: {e}. "
...
def benchmark_operation(
    ...
    return {
        ...
        "median_time": tf.nn.compute_average_loss(
            tf.nn.top_k(times, k=num_iterations // 2).values
        ).numpy(),
    }
...
def optimized_dense(
    ...
    with tf.name_scope(name or "sm120_dense"):
        ...
        kernel = tf.Variable(
            tf.keras.initializers.get(kernel_initializer)(kernel_shape), name="kernel"
        )
