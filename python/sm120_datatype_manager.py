...
class SM120TypeManager:
    ...
    def _initialize_gpu_capabilities(self):
        ...
        for dtype, info in SM120TypeProperties.TYPE_INFO.items():
            ...
            if self._gpu_compute_capability[0] > required_sm[0] or (
                self._gpu_compute_capability[0] == required_sm[0]
                and self._gpu_compute_capability[1] >= required_sm[1]
            ):
    ...
    def get_memory_usage_estimate(
        self, tensor_shape: Tuple[int, ...], dtype: Union[SM120DataType, str, tf.DType]
    ) -> int:
    ...
    def print_type_support_summary(self):
        ...
        for dtype in SM120DataType:
            ...
            print(
                f"{dtype.value:<12} {properties.get('size_bytes', 0):<6} "
...
def auto_cast(
    tensor: tf.Tensor, operation: str = "general", preserve_precision: bool = False
) -> tf.Tensor:
    """Automatically cast tensor to optimal data type."""
...
def create_optimized_tensor(
    shape: Tuple[int, ...], operation_type: str = "general", initializer: str = "zeros"
) -> tf.Tensor:
