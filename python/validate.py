...
class SM120Validator:
    ...
    def validate_system_requirements(self) -> bool:
        ...
        if python_version >= (3, 9):
            ...
            log_error(f"Python version: {python_version.major}.{python_version.minor}.{python_version.micro} (3.9+ required)")
            success = False
    ...
    def validate_sm120_operations(self) -> bool:
        ...
        try:
            ...
            log_success(f"Device info retrieved: {len(device_info.get('devices', []))} devices")
            ...
            if is_available:
                ...
                if result.shape.as_list() == expected_shape:
                    ...
                    log_error(f"SM120 matrix multiplication shape mismatch: got {result.shape}, expected {expected_shape}")
                    success = False
    ...
    def validate_performance(self) -> bool:
        ...
        try:
            ...
            start = time.time()
            for _ in range(10):
                result_tf = tf.matmul(A, B)
            tf_time = time.time() - start
            
            # Time SM120
            start = time.time()
            for _ in range(10):
                result_sm120 = sm120_ops.sm120_matmul(A, B)
            ...
            if sm120_time < tf_time:
                ...
                log_warning("SM120 did not show performance improvement (may be due to small problem size)")
