    ...
    def benchmark_convolution(self, configs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        with tf.device(self.device):
            for i, config in enumerate(configs):
                ...
                print(f"  Config {i+1}: {batch_size}x{input_shape[0]}x{input_shape[1]}x{input_shape[2]} -> {filters} filters")
            ...
            with tf.device(self.device):
                for size in sizes:
                    ...
                    model = tf.keras.Sequential([
                        ...
                        tf.keras.layers.Dense(size//4, activation='softmax', 
    ...
    def benchmark_reduction_operations(self, shapes: List[Tuple[int, ...]]) -> Dict[str, Any]:
        ...
        with tf.device(self.device):
            for shape in shapes:
                ...
                print(f"    Avg time per operation: {np.mean([r['time_ms'] for r in shape_results.values()]):.2f}ms")
