...
class TestSM120Operations(unittest.TestCase):
    ...
    def create_test_matrices(self, shape: Tuple[int, int], dtype: tf.DType) -> Tuple[tf.Tensor, tf.Tensor]:
    ...
    def assert_tensors_close(self, actual: tf.Tensor, expected: tf.Tensor, 
...
class TestAdvancedMatMul(TestSM120Operations):
        ...
        for opt_level in [0, 1, 2]:
            with self.subTest(optimization_level=opt_level):
                result = sm120_ops.advanced_matmul(
                    a, b, optimization_level=opt_level)
    ...
    def test_large_matrices(self):
        ...
        print(f"Large matmul ({shape}): SM120={sm120_time:.4f}s, "
...
class TestAdvancedConv2D(TestSM120Operations):
    ...
    def create_conv_test_data(self, batch_size: int, height: int, width: int, 
                            in_channels: int, out_channels: int, 
                            kernel_size: int, dtype: tf.DType) -> Tuple[tf.Tensor, tf.Tensor]:
    ...
    def test_large_conv2d(self):
        ...
        print(f"Large conv2d: SM120={sm120_time:.4f}s, "
              f"Reference={ref_time:.4f}s, Speedup={ref_time/sm120_time:.2f}x")

class TestFlashAttention(TestSM120Operations):
    """Tests for Flash Attention implementation."""
    
    def create_attention_test_data(self, batch_size: int, num_heads: int, 
                                 seq_len: int, head_dim: int, 
                                 dtype: tf.DType) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
        """Create test data for attention."""
        queries = tf.random.normal([batch_size, num_heads, seq_len, head_dim], 
                                 dtype=dtype, stddev=0.1)
        keys = tf.random.normal([batch_size, num_heads, seq_len, head_dim], 
                              dtype=dtype, stddev=0.1)
        values = tf.random.normal([batch_size, num_heads, seq_len, head_dim], 
                                dtype=dtype, stddev=0.1)
        return queries, keys, values
    
    def reference_attention(self, queries: tf.Tensor, keys: tf.Tensor, 
                          values: tf.Tensor, scale: float) -> Tuple[tf.Tensor, tf.Tensor]:
    ...
    def test_long_sequence_attention(self):
        ...
        start_time = time.time()
        output_sm120, _ = sm120_ops.flash_attention(
            queries, keys, values, scale=scale)
        ...
        print(f"Long sequence attention (seq_len={seq_len}): "
...
class TestPerformanceBenchmarks(TestSM120Operations):
    ...
    def test_matmul_performance_scaling(self):
        ...
        for size in sizes:
            ...
            benchmark_results = sm120_ops.benchmark_operation(
                sm120_ops.advanced_matmul, a, b,
                num_iterations=10, warmup_iterations=3
            )
            ...
            print(f"MatMul {size}x{size}: {benchmark_results['mean_time']*1000:.2f}ms, "
                  f"{gflops:.1f} GFLOPS")
        
        # Verify performance scaling
        self.assertGreater(results[-1]['gflops'], results[0]['gflops'],
    ...
    def test_conv2d_performance_scaling(self):
        ...
        for i, (batch_size, height, width, in_ch, out_ch, kernel_size) in enumerate(configs):
            ...
            benchmark_results = sm120_ops.benchmark_operation(
                sm120_ops.advanced_conv2d, input_tensor, filter_tensor, 1, "SAME",
                num_iterations=5, warmup_iterations=2
            )
...
if __name__ == '__main__':
    ...
    if SM120_AVAILABLE:
        ...
        print(f"Compatible devices: {len([d for d in device_info['devices'] if d.get('sm120_compatible', False)])}")
