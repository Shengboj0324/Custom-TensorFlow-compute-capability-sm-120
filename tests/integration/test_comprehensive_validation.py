    ...
    def test_gpu_availability(self):
        ...
        for i, gpu in enumerate(gpus):
            try:
                ...
                ), f"GPU {i} compute capability too low: {compute_capability}"
...
class TestBasicOperations:
    ...
    def test_matrix_multiplication(self, batch_size, data_type):
        ...
        with tf.device("/GPU:0"):
            ...
            assert (
                speedup >= TEST_CONFIG["performance_threshold"]
    ...
    def test_convolution(self, batch_size, channels):
        ...
        with tf.device("/GPU:0"):
            ...
            # SM120 operation
            start_time = time.perf_counter()
            result_sm120 = sm120_ops.conv2d(
                input_tensor, filter_tensor, strides=[1, 1, 1, 1], padding="SAME"
            )
            sm120_time = time.perf_counter() - start_time
...
class TestKerasLayerIntegration:
    ...
    def test_sm120_conv2d_layer(self):
        ...
        # Create layers
        sm120_layer = SM120Conv2D(
            filters, (3, 3), padding="same", activation="relu", use_sm120=True
        )
